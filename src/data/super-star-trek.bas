...
490 Q1=INT(RND(0)*7.98+1.01):Q2=INT(RND(0)*7.98+1.01):S1=INT(RND(0)*7.98+1.01):S2=INT(RND(0)*7.98+1.01)
...
1040 G(I,J)=K3*100+B3*10+INT(RND(0)*7.98+1.01):NEXT J:NEXT I:IF K9>T9 THEN T9=K9+1
1100 IF B9<>0 THEN 1200
1150 IF G(Q1,Q2)<200 THEN G(Q1,Q2)=G(Q1,Q2)+120:K9=K9+1
1160 B9=1:G(Q1,Q2)=G(Q1,Q2)+10:Q1=INT(RND(0)*7.98+1.01):Q2=INT(RND(0)*7.98+1.01)
...
1780 GOSUB 8670:K(I,1)=R1:K(I,2)=R2:K(I,3)=S9*(0.5+RND(0)):NEXT I
...
2880 NEXT I:IF RND(0)>.2 THEN 3070
2910 R1=INT(RND(0)*7.98+1.01):IF RND(0)>=.6 THEN 3000
...
4410 E=E-X:IF D(7)<0 THEN X=X*RND(0)
4450 H1=INT(X/K3):FOR I=1TO3:IF K(I,3)<=0 THEN 4670
4480 H=INT((H1/SQR((K(I,1)-S1)*(K(I,1)-S1)+(K(I,2)-S2)*(K(I,2)-S2)))*(RND(0)+2)):IF H>.15*K(I,3) THEN 4530
...
6060 H=INT((K(I,3)/SQR((K(I,1)-S1)*(K(I,1)-S1)+(K(I,2)-S2)*(K(I,2)-S2)))*(2+RND(0))):S=S-H:K(I,3)=K(I,3)/(3+RND(0))
...
6140 R1=INT(RND(0)*7.98+1.01):D(R1)=D(R1)-H/S-.5*RND(0):GOSUB 8790
...
6400 PRINT "YOUR EFFICIENCY RATING IS";1000*(K7/(T-T0))*(K7/(T-T0)):GOTO 6290
...
8460 PRINT "DISTANCE =";SQR(X*X+A*A):IF H8=1 THEN 1990
...
8590 R1=INT(RND(0)*7.98+1.01):R2=INT(RND(0)*7.98+1.01):A$="   ":Z1=R1:Z2=R2:GOSUB 8830:IF Z3=0 THEN 8590
8600 RETURN
