...
export async function runAllPhase1Tests(logFn) {
  ...
  runSuite("Z80CPU - Load Instructions", () => {
    runTest(
      "should execute LD A, n",
      () => {
        const { cpu, memory, logMemoryRange } = setupCPU();
        if (verboseLogging) {
          logFn(
            `        📝 Test Setup: Preparing instruction at 0x0000`,
            "info"
          );
        }
    ...
    runTest(
      "should execute LD HL, nn",
      () => {
        const { cpu, memory, logMemoryRange } = setupCPU();
        if (verboseLogging) {
          logFn(
            `        📝 Test Setup: Preparing LD HL, nn instruction`,
            "info"
          );
        }
    ...
    runTest(
      ...
      () => {
        ...
        if (verboseLogging) {
          logMemoryRange(0x0000, 0x0001, "instruction bytes");
          logFn(
            `        📝 Instruction: LD (HL), 0xAA where HL=0x5000`,
            "info"
          );
  ...
  runSuite("Z80CPU - Control Flow", () => {
    runTest(
      "should execute JP nn (unconditional jump)",
      () => {
    ...
    runTest(
      "should execute JP Z, nn when zero flag is set",
      () => {
        const { cpu, memory } = setupCPU();
        cpu.flagZ = 1;
        memory[0x0000] = 0xca;
  ...
  runSuite("Z80CPU - Advanced ED Prefix - Block Transfer Operations", () => {
    runTest(
      "should execute LDI (load and increment)",
      () => {
        const { cpu, memory, logMemoryRange } = setupCPU();
        if (verboseLogging) {
    ...
    runTest(
      "should execute LDIR (load, increment, repeat)",
      () => {
        const { cpu, memory, logMemoryRange } = setupCPU();
        if (verboseLogging) {
          logFn(
        ...
        if (verboseLogging) {
          ...
          logFn(
            `        📝 Will execute LDIR until BC=0 (3 iterations)`,
            "info"
          );
        }
        let cycles = 0;
    ...
    runTest(
      "should execute LDD (load and decrement)",
      () => {
        const { cpu, memory } = setupCPU();
        cpu.HL = 0x4002;
        cpu.DE = 0x5002;
    ...
    runTest(
      "should execute CPI (compare and increment)",
      () => {
        const { cpu, memory } = setupCPU();
        cpu.HL = 0x4000;
        cpu.BC = 0x0005;
  ...
  runSuite("Z80CPU - Advanced ED Prefix - Extended Load Instructions", () => {
    ...
    runTest(
      ...
      {
        assembly: "LD (0x5000), BC",
        opcode: "0xED 0x43 0x00 0x50",
      }
    );
  });
  ...
  runSuite("Z80CPU - Advanced ED Prefix - Extended Arithmetic", () => {
    ...
    runTest(
      "should execute NEG (negate accumulator)",
      () => {
        const { cpu, memory } = setupCPU();
        cpu.registers.A = 0x42;
        memory[0x0000] = 0xed;
  ...
  runSuite("Z80CPU - Advanced ED Prefix - Interrupt Handling", () => {
    runTest(
      "should execute RETI (return from interrupt)",
      () => {
        const { cpu, memory } = setupCPU();
        cpu.IFF1 = false;
        cpu.IFF2 = true;
    ...
    runTest(
      "should execute RETN (return from NMI)",
      () => {
        const { cpu, memory } = setupCPU();
        cpu.IFF1 = false;
        cpu.IFF2 = true;
    ...
    runTest(
      "should execute IM 0 (interrupt mode 0)",
      () => {
        const { cpu, memory } = setupCPU();
        memory[0x0000] = 0xed;
        memory[0x0001] = 0x46;
    ...
    runTest(
      "should execute IM 1 (interrupt mode 1)",
      () => {
        const { cpu, memory } = setupCPU();
        memory[0x0000] = 0xed;
        memory[0x0001] = 0x56;
    ...
    runTest(
      "should execute IM 2 (interrupt mode 2)",
      () => {
        const { cpu, memory } = setupCPU();
        memory[0x0000] = 0xed;
        memory[0x0001] = 0x5e;
  ...
  runSuite("Z80CPU - Advanced CB Prefix - Rotate and Shift Operations", () => {
    runTest(
      "should execute RLC r (rotate left circular)",
      () => {
        const { cpu, memory } = setupCPU();
